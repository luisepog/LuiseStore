...
- (NSMutableArray*)specifiers
{
	if(!_specifiers)
	{
		...
		NSArray *inactiveBundlePaths = luiseStoreInactiveInstalledAppBundlePaths();
		...
		PSSpecifier* advancedLinkSpecifier = [PSSpecifier preferenceSpecifierNamed:@"Advanced"
										...
										cell:PSLinkListCell
										edit:nil];
		advancedLinkSpecifier.detailControllerClass = [LSSettingsAdvancedListController class];
		...
		PSSpecifier* donateSpecifier = [PSSpecifier preferenceSpecifierNamed:@"Donate"
										...
										cell:PSLinkListCell
										edit:nil];
		donateSpecifier.detailControllerClass = [LSDonateListController class];
...
{
	[LSInstallationController installLdid];
}
...
- (void)enableDevModePressed
{
	...
	if (ret == 0) {
		...
		[LSPresentationDelegate presentViewController:rebootNotification animated:YES completion:nil];
	} else {
...
- (void)installPersistenceHelperPressed
{
	...
	[[LSApplicationWorkspace defaultWorkspace] enumerateApplicationsOfType:1 block:^(LSApplicationProxy* appProxy)
	{
		if(appProxy.installed && !appProxy.restricted)
		{
			...
			{
				NSURL* luiseStoreMarkURL = [appProxy.bundleURL.URLByDeletingLastPathComponent URLByAppendingPathComponent:LS_ACTIVE_MARKER];
				if(![luiseStoreMarkURL checkResourceIsReachableAndReturnError:nil])
				{
	...
	[LSPresentationDelegate presentViewController:selectAppAlert animated:YES completion:nil];
}

- (void)transferAppsPressed
{
	UIAlertController *confirmationAlert = [UIAlertController alertControllerWithTitle:@"Transfer Apps" message:[NSString stringWithFormat:@"This option will transfer %zu apps from "OTHER_APP_NAME@" to "APP_NAME@". Continue?", luiseStoreInactiveInstalledAppBundlePaths().count] preferredStyle:UIAlertControllerStyleAlert];
	...
	{
		[LSPresentationDelegate startActivity:@"Transfering"];
		dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^
		{
			...
			dispatch_async(dispatch_get_main_queue(), ^
			{
				[LSPresentationDelegate stopActivityWithCompletion:^
				{
					...
					if (transferRet != 0) {
						NSArray *remainingApps = luiseStoreInactiveInstalledAppBundlePaths();
...
- (void)setURLSchemeEnabled:(id)value forSpecifier:(PSSpecifier*)specifier
{
	...
	[LSPresentationDelegate presentViewController:rebuildNoticeAlert animated:YES completion:nil];
}
...
{
	NSUserDefaults* lsDefaults = luiseStoreUserDefaults();
	[lsDefaults setObject:value forKey:[specifier propertyForKey:@"key"]];
}

- (NSObject*)readPreferenceValue:(PSSpecifier*)specifier
{
	NSUserDefaults* lsDefaults = luiseStoreUserDefaults();
	NSObject* toReturn = [lsDefaults objectForKey:[specifier propertyForKey:@"key"]];
	if(!toReturn)
	{
...
- (NSMutableArray*)argsForUninstallingLuiseStore
{
	...
	NSNumber* uninstallationMethodToUseNum = [luiseStoreUserDefaults() objectForKey:@"uninstallationMethod"];
