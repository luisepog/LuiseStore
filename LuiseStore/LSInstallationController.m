...
@implementation LSInstallationController
...
+ (void)handleAppInstallFromFile:(NSString*)pathToIPA forceInstall:(BOOL)force completion:(void (^)(BOOL, NSError*))completionBlock
{
	...
	{
		[LSPresentationDelegate startActivity:@"Installing"];
		dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^
		{
			// Install IPA
			NSString* log;
			int ret = [[LSApplicationsManager sharedInstance] installIpa:pathToIPA force:force log:&log];

			NSError* error;
			if(ret != 0)
			{
				error = [[LSApplicationsManager sharedInstance] errorForCode:ret];
			}
			...
			dispatch_async(dispatch_get_main_queue(), ^
			{
				[LSPresentationDelegate stopActivityWithCompletion:^
				{
					if (ret == 0) {
						...
						[LSPresentationDelegate presentViewController:warningAlert animated:YES completion:nil];
					} else {
						// unrecoverable error
						UIAlertController* errorAlert = [UIAlertController alertControllerWithTitle:[NSString stringWithFormat:@"Install Error %d", ret] message:[error localizedDescription] preferredStyle:UIAlertControllerStyleAlert];
...
{
	NSNumber* installAlertConfigurationNum = [luiseStoreUserDefaults() objectForKey:@"installAlertConfiguration"];
	...
	LSAppInfo* appInfo = [[LSAppInfo alloc] initWithIPAPath:pathToIPA];
	[appInfo loadInfoWithCompletion:^(NSError* error)
	{
		dispatch_async(dispatch_get_main_queue(), ^
		{
			if(!error)
			{
				...
				[LSPresentationDelegate presentViewController:installAlert animated:YES completion:nil];
			}
			else
			{
...
+ (void)handleAppInstallFromRemoteURL:(NSURL*)remoteURL completion:(void (^)(BOOL, NSError*))completionBlock
{
	...
	dispatch_async(dispatch_get_main_queue(), ^
	{
			...
			dispatch_async(dispatch_get_main_queue(), ^
			{
				[LSPresentationDelegate stopActivityWithCompletion:^
				{
					if(error)
					{
						...
						[LSPresentationDelegate presentViewController:errorAlert animated:YES completion:^
						{
							if(completionBlock) completionBlock(NO, error);
						}];
					}
		...
		[LSPresentationDelegate startActivity:@"Downloading" withCancelHandler:^
		{
			[downloadTask cancel];
		}];
