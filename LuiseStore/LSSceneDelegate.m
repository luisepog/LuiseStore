...
@implementation LSSceneDelegate
	...
	for(UIOpenURLContext* context in URLContexts)
	{
		...
		if(url)
		{
			if([url isFileURL])
			{
				...
				{
					[LSInstallationController presentInstallationAlertIfEnabledForFile:url.path isRemoteInstall:NO completion:^(BOOL success, NSError* error){
						doneBlock(NO);
					}];
				}
			...
			else if([url.scheme isEqualToString:@"apple-magnifier"])
			{
				...
				if([components.host isEqualToString:@"install"])
				{
					...
					if(URLStringToInstall && [URLStringToInstall isKindOfClass:NSString.class])
					{
						...
						[LSInstallationController handleAppInstallFromRemoteURL:URLToInstall completion:nil];
					}
				}
...
- (void)handleEnableJITForBundleID:(NSString *)appId
{
	...
	if(!didOpen)
	{
		...
		// we don't have LSAppInfo here so we cannot check the registration state
...
- (void)handleLdidCheck
{
	...
	//if (@available(iOS 16, *)) {} else {
		NSString* lsAppPath = [NSBundle mainBundle].bundlePath;

		NSString* ldidPath = [lsAppPath stringByAppendingPathComponent:@"ldid"];
		NSString* ldidVersionPath = [lsAppPath stringByAppendingPathComponent:@"ldid.version"];
