...
int spawnRoot(NSString* path, NSArray* args, NSString** stdOut, NSString** stdErr)
{
	...
	if(stdOut || stdErr)
	{
		logQueue = dispatch_queue_create("vn.antiac01.LuiseStore.LogCollector", NULL);
...
NSArray* luiseStoreInstalledAppContainerPathsInternal(NSString *marker)
{
	...
	for(NSString* container in containers)
	{
		...
		{
			NSString* luiseStoreMark = [containerPath stringByAppendingPathComponent:marker];
			if([[NSFileManager defaultManager] fileExistsAtPath:luiseStoreMark])
			{
				NSString* luiseStoreApp = [containerPath stringByAppendingPathComponent:@"LuiseStore.app"];
				NSString* luiseStoreLiteApp = [containerPath stringByAppendingPathComponent:@"LuiseStoreLite.app"];
				if(![[NSFileManager defaultManager] fileExistsAtPath:luiseStoreApp] && ![[NSFileManager defaultManager] fileExistsAtPath:luiseStoreLiteApp])
				{
...
NSArray *luiseStoreInstalledAppContainerPaths(void)
{
	return luiseStoreInstalledAppContainerPathsInternal(LS_ACTIVE_MARKER);
}

NSArray* luiseStoreInstalledAppBundlePathsInternal(NSString *marker)
{
	NSMutableArray* appPaths = [NSMutableArray new];
	for(NSString* containerPath in luiseStoreInstalledAppContainerPathsInternal(marker))
	{
...
NSArray *luiseStoreInstalledAppBundlePaths(void)
{
	return luiseStoreInstalledAppBundlePathsInternal(LS_ACTIVE_MARKER);
}

NSArray *luiseStoreInactiveInstalledAppBundlePaths(void)
{
	return luiseStoreInstalledAppBundlePathsInternal(LS_INACTIVE_MARKER);
}

NSString* luiseStorePath()
{
	NSError* mcmError;
...
NSString* luiseStoreAppPath()
{
	return [luiseStorePath() stringByAppendingPathComponent:@"LuiseStore.app"];
}
...
LSApplicationProxy* findPersistenceHelperApp(PERSISTENCE_HELPER_TYPE allowedTypes)
{
	...
	void (^searchBlock)(LSApplicationProxy* appProxy) = ^(LSApplicationProxy* appProxy)
	{
		if(appProxy.installed && !appProxy.restricted)
		{
			...
			{
				NSURL* luiseStorePersistenceMarkURL = [appProxy.bundleURL URLByAppendingPathComponent:@".LuiseStorePersistenceHelper"];
				if([luiseStorePersistenceMarkURL checkResourceIsReachableAndReturnError:nil])
				{
					outProxy = appProxy;
				}
			}
...
{
    NSObject *lsPreAppliedExploitType = infoDict[@"TSPreAppliedExploitType"];
    if([lsPreAppliedExploitType isKindOfClass:[NSNumber class]])
    {
        NSNumber *lsPreAppliedExploitTypeNum = (NSNumber *)lsPreAppliedExploitType;
        int exploitTypeInt = [lsPreAppliedExploitTypeNum intValue];
    ...
    NSObject *lsBundleIsPreSigned = infoDict[@"TSBundlePreSigned"];
    if([lsBundleIsPreSigned isKindOfClass:[NSNumber class]])
    {
        NSNumber *lsBundleIsPreSignedNum = (NSNumber *)lsBundleIsPreSigned;
        if([lsBundleIsPreSignedNum boolValue] == YES)
        {
